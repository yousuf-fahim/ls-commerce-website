...
function StatBlock({ value, label, delay }: { value: string; label: string; delay: number }) {
  return (
    ...
    >
      <div className="text-[28px] font-medium tracking-tight text-white">{value}</div>
      <div className="mt-1 text-[12px] tracking-wide text-zinc-500">{label}</div>
    </motion.div>
  )
}
...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-zinc-950 px-6 pt-20 pb-16 md:pt-28 md:pb-24">
      {/* Grid background */}
      ...
      <div className="relative mx-auto max-w-[1100px]">
        <div className="grid items-center gap-12 md:grid-cols-2 md:gap-16">
          ...
          <div>
            ...
            <motion.h1
              ...
              className="text-[clamp(2rem,4.5vw,3rem)] font-medium leading-[1.15] tracking-tight text-white"
            >
              Digitale Lösungen.
              <br />
        ...
        <div className="mt-16 grid grid-cols-3 border-t border-zinc-800 pt-10 md:mt-20">
          <StatBlock value="CH" label="Firmensitz Schweiz" delay={0.6} />
          <StatBlock value="3" label="Geschäftsfelder" delay={0.7} />
          <StatBlock value="Int." label="Internationale Reichweite" delay={0.8} />
        </div>
      </div>
    </section>
  )
